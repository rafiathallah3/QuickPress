// src/content.ts
var HIGHLIGHT_CLASS = "quickpress-highlight";
var BUTTON_HIGHLIGHT_CLASS = "quickpress-button-highlight";
var BUTTON_ACTIVE_CLASS = "quickpress-button-highlight-active";
var OVERLAY_ID = "quickpress-overlay";
var SHORTCUT_KEYS = "1234567890abcdefghijklmnopqrstuvwxyz";
...
var shortcutConfig = DEFAULT_SHORTCUT;
...
var applyShortcutConfig = (value) => {
  shortcutConfig = sanitizeShortcut(value);
};
var loadShortcutConfig = () => {
  chrome.storage.sync.get(SHORTCUT_STORAGE_KEY, (result) => {
    applyShortcutConfig(result?.[SHORTCUT_STORAGE_KEY]);
  });
};
var normalizedEventKey = (value) => value.length === 1 ? value.toLowerCase() : value.toLowerCase();
var matchesShortcut = (event) => {
  const key = normalizedEventKey(event.key);
  return Boolean(event.ctrlKey) === Boolean(shortcutConfig.ctrl) && Boolean(event.altKey) === Boolean(shortcutConfig.alt) && Boolean(event.shiftKey) === Boolean(shortcutConfig.shift) && Boolean(event.metaKey) === Boolean(shortcutConfig.meta) && key === shortcutConfig.key;
};
loadShortcutConfig();
...
var collectTargets = () => {
  ...
  return nodes.filter(
    (node) => node instanceof HTMLElement && node.offsetParent !== null && !(node instanceof HTMLButtonElement && node.disabled)
  );
};
var resetHighlights = () => {
...
var createListItem = (button, index, shortcut) => {
  ...
  const label = button.innerText?.trim() || button.getAttribute("aria-label") || button.getAttribute("title") || button.id || (button instanceof HTMLAnchorElement ? button.getAttribute("href") : void 0) || `Element ${index + 1}`;
...
var openOverlay = () => {
  ...
  instructions.textContent = "Search by text, press the shortcut key, or click an item. Press Esc to cancel.";
...
document.addEventListener(
  ...
  (event) => {
    ...
    if (state.overlay) {
      ...
      if (!isTypingField(event.target) && SHORTCUT_KEYS.includes(key)) {
